"""
IOI Circuit Discovery with KL Budget Constraint + Momentum-Based Exploration (GPT-2).
...
Usage:
    ...
    python ioi_gpt2_kl_budget.py --kl-budget 0.5

    # With Weights & Biases logging (comprehensive tracking + visualization)
    python ioi_gpt2_kl_budget.py --kl-budget 0.5 --wandb --wandb-project my-circuits

    # With task loss for better task accuracy
    python ioi_gpt2_kl_budget.py --kl-budget 0.5 --use-task-loss --task-lambda 1.0

    # Ultra-aggressive exploration (go wild!)
    python ioi_gpt2_kl_budget.py --kl-budget 0.5 --initial-lambda 5.0 --lambda-add-inc 0.15

    # More conservative (gentler exploration)
    python ioi_gpt2_kl_budget.py --kl-budget 0.5 --initial-lambda 1.0 --lambda-add-inc 0.04

    # Full setup with W&B tracking
    python ioi_gpt2_kl_budget.py \\
        --kl-budget 0.5 \\
        --use-task-loss \\
        --wandb \\
        --wandb-project circuit-discovery \\
        --wandb-name gpt2-ioi-kl05 \\
        --wandb-tags gpt2 ioi momentum

    # Dry run for testing
    python ioi_gpt2_kl_budget.py --kl-budget 0.5 --dry-run
"""
...
@dataclass
class KLBudgetGPT2PruningConfig(PruningConfig):
    """Base config for KL budget training on GPT-2."""
...
def compute_component_sparsity(model) -> Dict[str, float]:
    ...
    - mlp_hidden: Hidden/intermediate layer neurons
    - mlp_output: Output layer neurons
    - attention_blocks: Whole attention block gates
    - mlp_blocks: Whole MLP block gates
    - embedding: Embedding gate (GPT-2 specific)
    ...
    for name, module in model.named_modules():
        if isinstance(module, HardConcreteGate):
            # Categorize by actual gate names from gpt2_circuit.py
            comp_type = None
            ...
            elif 'embedding_gate' in name:
                if pruning_config.prune_embedding:
                    comp_type = 'embedding'
...
def compute_layer_sparsity(model) -> Dict[str, float]:
    ...
    GPT-2 layers live under transformer.h[i], so we look for 'h' in the path.
    """
    from models.l0 import HardConcreteGate
    ...
    for name, module in model.named_modules():
        if isinstance(module, HardConcreteGate):
            ...
            # Names look like: transformer.h.0.attn.head_gates
            parts = name.split('.')

            # Find the layer index - GPT-2 uses 'h' for the layer list
            layer_idx = None
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="KL Budget Circuit Discovery for GPT-2")
    parser.add_argument('--dry-run', action='store_true')
    parser.add_argument('--model', type=str, default='gpt2')
    parser.add_argument('--epochs', type=int, default=50000)
    parser.add_argument('--lr', type=float, default=1e-3)
    parser.add_argument('--batch-size', type=int, default=32)
    parser.add_argument('--save-dir', type=str, default='checkpoints_gpt2_kl_budget')
    ...
    print("="*80)
    print("  KL BUDGET CIRCUIT DISCOVERY (GPT-2)")
    print("="*80)
    ...
    pruning_config = KLBudgetGPT2PruningConfig()
    circuit_model = CircuitDiscoveryGPT2.from_pretrained_with_pruning(
        args.model, pruning_config
    ).to(DEVICE).eval()
    ...
    train_data = generate_ioi_data(NUM_TRAIN, tokenizer, seed=42)
    val_data = generate_ioi_data(NUM_VAL, tokenizer, seed=123)
    test_data = generate_ioi_data(NUM_TEST, tokenizer, seed=456)
    ...
    for epoch in tqdm(range(start_epoch, NUM_EPOCHS), desc="Training"):
        ...
        if (epoch + 1) % eval_freq == 0 or epoch == 0:
            ...
            # Use training KL (soft gates) for the scheduler — not eval KL (hard binary gates).
            # Hard-gate eval KL is structurally much larger: any gate with log_alpha slightly
            # below 0 (soft ≈ 0.35 during training) becomes hard 0 in eval, routing fully
            # through the corrupted stream and spiking KL by 10-100x. Using it here
            # permanently elevates ema_kl_loss and pins lambda at minimum.
            update_result = scheduler.step_update(
                step=total_steps,
                epoch=epoch + 1,
                accuracy=val_results['accuracy'],
                sparsity_rate=current_sparsity,
                kl_loss=float(avg_epoch_kl),  # soft-gate training KL (matches optimizer)
                component_sparsity=comp_sparsity,
                model=circuit_model,
            )

            # Log hard-gate eval KL separately for monitoring (higher than training KL by design)
            if args.wandb:
                try:
                    import wandb
                    if wandb.run:
                        wandb.log({'metrics/eval_kl_hard_gates': val_results['kl_div']}, step=epoch + 1)
                except Exception:
                    pass
