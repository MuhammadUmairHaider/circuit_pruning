"""
Gender Pronouns (GP) Circuit Discovery with KL Budget Constraint + Momentum-Based Exploration.
...
Usage:
    ...
    python gp_llama_kl_budget.py --kl-budget 0.5

    # With Weights & Biases logging (comprehensive tracking + visualization)
    python gp_llama_kl_budget.py --kl-budget 0.5 --wandb --wandb-project my-circuits

    # With task loss for better task accuracy
    python gp_llama_kl_budget.py --kl-budget 0.5 --use-task-loss --task-lambda 1.0

    # Ultra-aggressive exploration (go wild!)
    python gp_llama_kl_budget.py --kl-budget 0.5 --initial-lambda 5.0 --lambda-add-inc 0.15

    # More conservative (gentler exploration)
    python gp_llama_kl_budget.py --kl-budget 0.5 --initial-lambda 1.0 --lambda-add-inc 0.04

    # Full setup with W&B tracking
    python gp_llama_kl_budget.py \\
        --kl-budget 0.5 \\
        --use-task-loss \\
        --flash-attn \\
        --wandb \\
        --wandb-project circuit-discovery \\
        --wandb-name llama-gp-kl05 \\
        --wandb-tags llama gp momentum

    # Dry run for testing
    python gp_llama_kl_budget.py --kl-budget 0.5 --dry-run
"""
...
class KLBudgetScheduler:
    ...
    def plot_training_dynamics(self, save_dir: str):
        ...
        plt.suptitle('Training Dynamics Dashboard (GP)', fontsize=18, fontweight='bold', y=0.995)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="KL Budget Circuit Discovery - Gender Pronouns")
    ...
    parser.add_argument('--save-dir', type=str, default='checkpoints_llama_gp_kl_budget')

    # Dataset path
    parser.add_argument('--dataset-path', type=str, default='./data/datasets/gp/',
                        help='Path to the GP dataset directory')
    ...
    print("="*80)
    print("  KL BUDGET CIRCUIT DISCOVERY - GENDER PRONOUNS")
    print("="*80)
    ...
    train_data = load_or_generate_gp_data(
        dataset_path=args.dataset_path, split="train", num_samples=NUM_TRAIN
    )
    val_data = load_or_generate_gp_data(
        dataset_path=args.dataset_path, split="validation", num_samples=NUM_VAL
    )
    test_data = load_or_generate_gp_data(
        dataset_path=args.dataset_path, split="test", num_samples=NUM_TEST
    )
    ...
    for epoch in tqdm(range(start_epoch, NUM_EPOCHS), desc="Training"):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            # KL loss - computed at the single prediction position (before pronoun)
            total_kl = 0
            num_valid_samples = 0
            ...
            # Task loss (optional) - logit difference between target and distractor pronoun
            task_loss = 0.0
            if scheduler.config.use_task_loss:
                batch_indices = torch.arange(batch_size, device=DEVICE)
                pred_positions = batch['prefix_length'] - 1
                logit_target = outputs.logits[batch_indices, pred_positions, batch['target_token']].float()
                logit_distractor = outputs.logits[batch_indices, pred_positions, batch['distractor_token']].float()
                task_loss = F.relu(4.0 - (logit_target - logit_distractor)).mean() * scheduler.config.task_lambda
        ...
        if (epoch + 1) % args.checkpoint_interval == 0:
            ...
            _sched_keys = [
                ...
                'kl_window', 'sparsity_window', 'step', 'epoch',
            ]
        ...
        # Validation frequency: every 5 epochs early, then 10, then 20
        if epoch < 50:
            eval_freq = 5
        elif epoch < 100:
            eval_freq = 10
    ...
    # 3. Save raw test data for post-hoc analysis
