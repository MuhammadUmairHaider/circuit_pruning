...
#             return (self.log_alpha > cutoff).float()
...
class HardConcreteGate(nn.Module):
    def __init__(self, size: int, beta: float = 2.0/3.0, gamma: float = -0.1, zeta: float = 1.1, init_min=1.5, init_max=2.5):
        super().__init__()
        self.register_buffer("beta", torch.tensor(beta))
        self.register_buffer("gamma", torch.tensor(gamma))
        self.register_buffer("zeta", torch.tensor(zeta))
    ...
    def forward(self) -> torch.Tensor:
        # 1. Final Mode (Evaluation): Hard Threshold
        if self.final_mode:
            cutoff = self.beta * torch.log(-self.gamma / self.zeta)
            return (self.log_alpha > cutoff).float()
        
        # 2. Training Mode: Sample or Use Mean
        if self.training:
            # Sample noise
            u = torch.rand_like(self.log_alpha).clamp(1e-8, 1.0 - 1e-8)
            # Log-ratio (Gumbel-Softmax trick)
            s = torch.sigmoid((torch.log(u) - torch.log(1 - u) + self.log_alpha) / self.beta)
        else:
            # Deterministic expectation for validation
            s = torch.sigmoid(self.log_alpha)
        
        # 3. Stretch and Clip (Standard Hard Concrete)
        s_stretched = s * (self.zeta - self.gamma) + self.gamma
        gate_soft = F.hardtanh(s_stretched, min_val=0, max_val=1)
        
        # 4. Apply Straight-Through Estimator (THE TRICK) 
        # Only apply this if we are training, or if you want validation to be binary too
        if self.training:
            gate_hard = (gate_soft > 0.5).float()
            # This line says: "Output binary, but backpropagate as if it were soft"
            return (gate_hard - gate_soft).detach() + gate_soft
            
        return gate_soft

    def get_sparsity_loss(self) -> torch.Tensor:
        # Calculate expected L0 norm (probability gate is open)
        p_open = torch.sigmoid(
            self.log_alpha - self.beta * torch.log(-self.gamma / self.zeta)
        )
        # CRITICAL FIX FROM PREVIOUS TURN: Use mean(), not sum()
        return p_open.mean()    
